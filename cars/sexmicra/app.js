...
var dweetName = 'PorcoPorcoBooking-Micra'
let nope = '🙅🏻‍♂️'
let yah = '🙋🏻‍♂️'
var postiMassimi = 4
...
function getDweet(dweetName) {
  ...
  dweetio.get_latest_dweet_for(dweetName, function(err, dweet) {
    ...
    Object.values(postiObject).forEach(seat => {
      if (seat != '') {
        let occuato = document.createElement('li')
        occuato.innerHTML = seat
        displayList.appendChild(occuato)
      }
...
const autenticateMe = () => {
  ...
  hiddenTextField.onchange = () => {
    ...
    if ((manculetHash ^ secondaChiave) === 904009744) {
      ...
      let returned = JSON.parse(defaultJson)
      sendDweet(returned)
