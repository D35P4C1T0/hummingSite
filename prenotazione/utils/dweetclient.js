...
;(function() {
  ...
  if (isNode) {
    ...
    if (require("fs").existsSync(LAST_THING_NAME)) {
      try {
        lastThing = require("fs")
          .readFileSync(LAST_THING_NAME)
          .toString()
      } catch (e) {}
    }
  } else {
    request = function(options, callback) {
      ...
      for (param_name in options.json) {
        params.push(
          param_name + "=" + encodeURIComponent(options.json[param_name])
        )
      }
  ...
  var dweetioClient = function() {
    ...
    self.set_server = function(server, strictSSL) {
      ...
      if (isNode) {
        ...
        else require("https").globalAgent.options.rejectUnauthorized = false
      }
    }
      ...
      if (currentThing) {
        ...
        request(
          ...
          function(err, response, body) {
            ...
            if (
              responseData["with"] &&
              responseData["with"].thing != currentThing
            ) {
    ...
    self.get_latest_dweet_for = function(thing, key, callback) {
      ...
      request(
        {
          url: createKeyedURL(
            DWEET_SERVER + "/get/latest/dweet/for/" + thing,
            key
          ),
          jar: true,
          timeout: REQUEST_TIMEOUT,
    ...
    self.get_key_for = function(account, thingname, callback) {
      request(
        {
          url: createKeyedURL(
            DWEET_SERVER + "/get/key/for/" + account + "/" + thingname
          ),
          jar: true,
          timeout: REQUEST_TIMEOUT,
          strictSSL: STRICT_SSL
    ...
    self.create_key_for = function(account, thingname, callback) {
      request(
        {
          url: createKeyedURL(
            DWEET_SERVER + "/create/key/for/" + account + "/" + thingname
          ),
          jar: true,
          timeout: REQUEST_TIMEOUT,
          strictSSL: STRICT_SSL
    ...
    self.get_keys_for_account = function(
      account,
      startPosition,
      endPosition,
      callback
    ...
    self.listen_for = function(thing, key, callback) {
      ...
      if (!socket) {
        if (isNode) {
          ...
          dweet_script_loader(
            [DWEET_SERVER + "/socket.io/socket.io.js"],
            function() {
              io = window.io
              createSocket()
            }
    ...
    self.lock = function(thing, lock, key, callback) {
      request(
        {
          url:
            DWEET_SERVER + "/lock/" + thing + "?lock=" + lock + "&key=" + key,
          jar: true,
          timeout: REQUEST_TIMEOUT,
          strictSSL: STRICT_SSL
        },
